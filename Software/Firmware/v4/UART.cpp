...
void deinitAllUARTPins() {
	...
	PORTB.DIR = PORTB.DIR & ~PIN3_bm; // RXD2 as INPUT pin, otherwise leakage
	...
	PORTB.PIN3CTRL = (0 << PORT_INVEN_bp) | (0 << PORT_PULLUPEN_bp) | (1 << PORT_ISC2_bp); // not inverted, no pull-up, disable input buffer (can't read input values), IMPORTANT, otherwise +30uA leakage
}
